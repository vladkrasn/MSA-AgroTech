...
Rel(camera1, camerareceiver, "Sends video feed", "Manufacturer-supported protocol")
Rel(camera2, camerareceiver, "Sends video feed", "Manufacturer-supported protocol")
Rel(camerareceiver, cameraio, "Sends video feed", "HTTP Live Streaming")
...
Rel(cameras3m, cameras3, "Stores video files", "S3 upload/download")
BiRel(cameraservice, cameradb, "Knows where cameras are", "PGSQL")
...
Rel(analytics, broker, "Sends analytical data", "Kafka")
Rel(analytics, analyticsdb, "Stores analytical data", "SQL")
@enduml
```
